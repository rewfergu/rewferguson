...
const tasksIntegration = () => {
  let config;
  return {
    name: 'RobertFerguson:tasks',

    hooks: {
      'astro:config:setup': async ({ config, logger, updateConfig, addWatchFile }) => {
        const buildLogger = logger.fork('robertferguson');

        const virtualModuleId = 'robertferguson:config';
        ...
        updateConfig({
          ...
          vite: {
            plugins: [
              {
                name: 'vite-plugin-robertferguson-config',
                resolveId(id) {
        ...
        buildLogger.info('RobertFerguson `src/config.yaml` has been loaded.');
      },
